<?php
// OPERATOR_REPLY_V8_RENDER_KP_588
...
function render_kp_588(){
  $ch=curl_init('https://onec-kp-realtime.onrender.com/api/max/test/kp-588');
  curl_setopt_array($ch,[CURLOPT_RETURNTRANSFER=>true,CURLOPT_HTTPHEADER=>['Accept: application/json','User-Agent: Shina-Moskva-MAX-Bridge/1.0'],CURLOPT_CONNECTTIMEOUT=>10,CURLOPT_TIMEOUT=>45]);
  $raw=curl_exec($ch); $code=(int)curl_getinfo($ch,CURLINFO_HTTP_CODE); $err=(string)curl_error($ch); curl_close($ch);
  $data=json_decode((string)$raw,true);
  if($code===200 && is_array($data) && !empty($data['text'])) return ['ok'=>true,'text'=>(string)$data['text'],'code'=>$code,'error'=>''];
  return ['ok'=>false,'text'=>'','code'=>$code,'error'=>$err!==''?$err:'invalid Render response'];
}
function send_kp_588($query,$token){
  ...
  bridge_log('KP588 render_code='.$kp['code'].' max_code='.$sent['code'].' error='.preg_replace('/\s+/u',' ',(string)$kp['error']));
  return $sent;
}
